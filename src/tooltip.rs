...
use crate::keybinds::get_actions_for_mode;
use crate::State;
use zellij_hud::text::visible_len;
use zellij_hud::tooltip_layout::{
    tooltip_position_size, tooltip_size, FRAME_COLS, FRAME_ROWS,
};
...
fn tooltip_coordinates_at_size(
    ...
    let (area_rows, area_cols) = tabs
        .iter()
        .find(|t| t.active)
        .map(|t| (t.display_area_rows, t.display_area_columns))
        .unwrap_or((24, 80));

    let (x, y, w, h) = tooltip_position_size(
        tt_rows, tt_cols, area_rows, area_cols, status_bar_enabled, position,
    );
